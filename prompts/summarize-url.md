...
- はてなブックマークの通常のページを読み取り、ブックマーク数とコメントを取得する。取得に失敗することがある
- **取得できなかった場合**：「はてなブックマーク情報：取得不可」と表示し、原因が明らかな場合のみ簡潔に記載する。原因が不明なら深追いしない。コメント関連のセクションは省略
...
はてなブックマークのページを確認し、全体的な反応のトーンと代表的なコメントを引用してください。はてブ URL もクリッカブルリンクで提示してください。
...
3. **はてなブックマークでの反応**
   ...
   - **情報取得状況に応じた表示**：
     - **取得不可の場合**：「はてなブックマーク情報：取得不可」と表示。原因が明らかな場合のみ簡潔に記載し、以下の項目は省略

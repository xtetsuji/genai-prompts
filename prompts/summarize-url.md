...
**URL が複数入力された場合は、各記事を個別に要約したあと、記事間の多角的な比較・分析を行ってください（詳細は「複数 URL モード」セクションを参照）。**
...
**2つ以上の URL が入力された場合は、以下の手順で処理してください。**

### ステップ 1：各記事の個別要約

各 URL に対して「出力セクション構成」に従った個別要約を行ってください。セクション見出しには `[記事 N]` のように番号を付けて区別してください（例: `## [記事 1] 記事情報`）。

### ステップ 2：複数記事の総合分析

全記事の個別要約が完了したあと、以下のセクションを追加してください。

#### 1. 記事間の比較（差異分析）

各記事が同じテーマ・トピックを扱っている場合に、以下の観点で比較してください：

- **主張・論点の比較**：各記事が強調している主張や視点の違い
- **根拠・データの比較**：引用されている統計・事例・専門家の違い
- **結論・提言の比較**：記事がどのような結論・行動を推奨しているかの違い
- 比較は表形式で示すと分かりやすい場合は積極的に活用してください

#### 2. 共通テーマの抽出

複数の記事に共通して登場するキーワード、主張、テーマをリストアップしてください。共通点が多い場合は「コンセンサス」として強調してください。

#### 3. 矛盾点・意見の相違

記事間で事実認識や主張が食い違う点を明示してください。矛盾が見つかった場合は、どちらの主張がより根拠に基づいているかも（根拠がある範囲で）コメントしてください。

#### 4. 時系列整理

記事の作成日時が異なる場合は、時系列で並べ直し、トピックに関する認識や状況がどのように変化しているかを整理してください。

#### 5. 情報の補完（クロスリファレンス）

ある記事が触れていない重要な情報が他の記事にある場合は、それを補完情報として示してください。複数記事を読むことで得られる「総合的な理解」を提供してください。

#### 6. 総合まとめ

全記事を踏まえた総合的な結論・まとめを箇条書きで提示してください。単なる各記事要約の羅列ではなく、複数の視点を統合した洞察を加えてください。

### 複数 URL モードの注意事項

- 各記事は独立して要約し、要約内でのハルシネーションは厳禁
- 比較・分析のセクションは取得できた情報のみに基づいて記載する
- 記事のテーマが全く異なる場合は、比較分析の代わりに「各記事は独立したトピックです」と明記し、個別要約のみを提供する
...
1. **記事情報**
   ...
   - Yahoo! ニュース転載記事（ニュースアグリゲーションでない記事執筆元サイトの場合のみ表示）
...
4. **Yahoo! ニュース記事内コメント**（Yahoo! ニュース転載記事が見つかった場合のみ表示）
   ...
   - Yahoo! ニュース記事内のコメント欄から代表的なコメントを表形式で抜粋
   - コメント選択基準は「はてなブックマークでの反応」と同様
...
- **ニュースアグリゲーションでない記事執筆元サイトの場合**：
  - 記事執筆元サイト（例：ITmedia、日経新聞、朝日新聞、読売新聞等）の記事 URL が提供された場合の対応
  - 記事タイトルや本文の特徴的なフレーズを使って Yahoo! ニュース（news.yahoo.co.jp）で転載記事を検索してください
...
`factcheck` コマンドは、要約した記事に含まれる主要な主張・数値・引用の正確性を検証します。

#### ファクトチェックのガイドライン

1. **主張の抽出**
   - 数値・統計、因果関係の主張、人物・組織の引用、過去の事実として記述されている内容を優先的に抽出

2. **各主張の検証**
   - 記事と独立した複数の信頼できる情報源で事実確認
   - 引用・発言が正確な文脈で使われているかも確認

3. **判定と出力**
   - 以下の判定スケールを使用：
     - ✅ 正確：複数の信頼できる情報源で確認済み
     - 🟡 おおむね正確：概ね正確だが部分的に不正確・誤解を招く表現がある
     - ⚠️ 根拠不明：事実確認ができない・情報源が確認できない
     - ❌ 不正確：複数の信頼できる情報源で誤りが確認された
     - 🔶 誤解を招く：技術的には正確だが文脈や切り取り方が誤解を招く
     - ℹ️ 確認不要：意見・推測・価値判断であり事実確認の対象外
   - 各主張について判定・根拠情報源・補足説明を簡潔に記載
   - 確認できない情報は推測で判定しない
...
| # | 主張の要旨 | 判定 | 根拠・補足 |
|---|-----------|------|-----------|
| 1 | ○○の利用者数は1000万人 | ✅ 正確 | 公式発表（参照: [URL]）と一致 |
| 2 | △△が原因で□□が発生 | 🔶 誤解を招く | 因果関係は専門家間で議論中（参照: [URL]）|
| 3 | ◇◇氏が「〜」と発言 | ⚠️ 根拠不明 | 該当発言を確認できる情報源が見つからない |

**総合評価**: [正確 / おおむね正確 / 部分的に問題あり / 重大な誤りあり]
```
